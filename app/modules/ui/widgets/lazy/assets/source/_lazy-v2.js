(function($){
  ...
  var LazyContainer = function($element, options){
    ...
    var init = function(){
      ...
      initContent = function($content){
        ...
        $element.trigger("lazy.initContent");
      },
    ...
    this.go = function(){
      ...
      this.confirmation().done(function(){
        if (options.modal) {
          ...
          $.lazyModal(modalOptions).load(url, options.method, options.data, options);
        } else {
          self.container().load(url, options.method, options.data, options);
        }

        $element.trigger("lazyLink.go");
      });
    };
  ...
  var LazyForm = function($element, options){
    ...
    var init = function(){
        ...
        $element.on("lazy.loaded", function(e){
  ...
  var LazyModal = function(options){
    ...
    var init = function(){
        ...
        self.$modalBody.on("lazy.render", onRender);
        self.$modalBody.on("lazy.beforeSend", onBeforeSend);
        self.$modal.one("hidden.bs.modal", onHide);
  ...
  $.lazyModal = function(options){
    ...
    var lazyModal = $("#" + options.id).data("lazyModal");

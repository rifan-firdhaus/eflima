...
<div class="widgets d-flex justify-content-between border-top flex-wrap">
    <?php foreach ($statuses AS $status): ?>
        <a href="<?= $searchModel->searchUrl($searchAction, [$searchModel->formName() => ['status_id' => $status['id']]]) ?>" class="widget <?= ($originalSearchModel->status_id == $status['id'] ? 'active' : '') ?> flex-grow-1 text-center">

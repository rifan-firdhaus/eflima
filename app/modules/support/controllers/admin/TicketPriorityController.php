...
class TicketPriorityController extends Controller
{
    ...
    public function behaviors()
    {
        ...
        $behaviors['access']['rules'] = [
            [
                ...
                'actions' => ['index'],
                'verbs' => ['GET'],
                'roles' => ['admin.setting.ticket.ticket-priority.list'],
            ],
            [
                'allow' => true,
                'actions' => ['add'],
                'verbs' => ['GET', 'POST'],
                'roles' => ['admin.setting.ticket.ticket-priority.add'],
            ],
            [
                'allow' => true,
                'actions' => ['update'],
                'verbs' => ['GET', 'POST', 'PATCH'],
                'roles' => ['admin.setting.ticket.ticket-priority.update'],
            ],
            [
                'allow' => true,
                'actions' => ['delete'],
                'verbs' => ['POST', 'DELETE'],
                'roles' => ['admin.setting.ticket.ticket-priority.delete'],
            ],
            [
                'allow' => true,
                'actions' => ['enable'],
                'verbs' => ['POST'],
                'roles' => ['admin.setting.ticket.ticket-priority.visibility'],
            ],
        ];
    ...
    {
        $model = $this->getModel($id, TicketPriority::class);

        if (!($model instanceof TicketPriority)) {
            return $model;
        }
    ...
    /**
     * @param integer               $id
     * @param string|TicketPriority $modelClass
     * @param null|Closure          $queryFilter
     *
     * @return string|Response|TicketPriority
     * @throws InvalidConfigException
     */
    public function getModel($id, $modelClass = TicketPriority::class, $queryFilter = null)
    {

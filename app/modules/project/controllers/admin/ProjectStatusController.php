...
class ProjectStatusController extends Controller
{
    ...
    public function behaviors()
    {
        ...
        $behaviors['access']['rules'] = [
            [
                ...
                'actions' => ['index'],
                'verbs' => ['GET'],
                'roles' => ['admin.setting.project.project-status.list'],
            ],
            [
                'allow' => true,
                'actions' => ['add'],
                'verbs' => ['GET', 'POST'],
                'roles' => ['admin.setting.project.project-status.add'],
            ],
            [
                'allow' => true,
                'actions' => ['update'],
                'verbs' => ['GET', 'POST', 'PATCH'],
                'roles' => ['admin.setting.project.project-status.update'],
            ],
            [
                'allow' => true,
                'actions' => ['delete'],
                'verbs' => ['POST', 'DELETE'],
                'roles' => ['admin.setting.project.project-status.delete'],
            ],
            [
                'allow' => true,
                'actions' => ['enable'],
                'verbs' => ['POST'],
                'roles' => ['admin.setting.project.project-status.visibility'],
            ],
        ];
    ...
    {
        $model = $this->getModel($id, ProjectStatus::class);

        if (!($model instanceof ProjectStatus)) {
            return $model;
        }
    ...
    /**
     * @param integer              $id
     * @param string|ProjectStatus $modelClass
     * @param null|Closure         $queryFilter
     *
     * @return string|Response|ProjectStatus
     * @throws InvalidConfigException
     */
    public function getModel($id, $modelClass = ProjectStatus::class, $queryFilter = null)
    {
